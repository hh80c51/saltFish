...
(function(){
     ...
     * @param data
     */
	commonUtil.jumpHdAccountMenu = function(menuUri, data){
	    if (!commonUtil.isEmpty(data)){
            containerUtil.add(CONSTANTS.CONTAINER_TRANSIENT_KEY, data);
        } 
	    var target = commonUtil.getRequestUrl("auction/" + menuUri);
        $("#includeContent").attr("file", target);
        commonUtil.includeOuterHtml(target);
    };
	/**
	...
	commonUtil.isLogin = function(){
		if(validUtil.validNotEmpty(cookieUtil.getCookie(CONSTANTS.SSO_TICKET)) && containerUtil.get(CONSTANTS.LOGIN_REG_USER) != undefined){
			...
			if (commonUtil.platform != "hongde"){
                commonUtil.loadUnreadWebMsg();
            } 
			if($(".nickNameView").text() == ''){
				$(".nickNameView").text("请输入昵称");
			}
			//物业现金缴费黑名单，不展示账户页面，直接跳转至物业销售查询页面
			if (!commonUtil.isEmpty(containerUtil.get(CONSTANTS.PROPERTY_PAYMENT_NO_CASH)) && containerUtil.get(CONSTANTS.PROPERTY_PAYMENT_NO_CASH) == 1){
				containerUtil.add("_transient_menu_uri","propertySales.html");
			}
			return true;
		}
		return false;
     ...
     * 登录验证,判断用户是否已经登录，如果没有登录直接跳到登录页
     */
    commonUtil.wapNeedLogin = function(){
        if(validUtil.validNotEmpty(cookieUtil.getCookie(CONSTANTS.SSO_TICKET)) &&
            (containerUtil.take(CONSTANTS.LOGIN_REG_USER) != undefined || containerUtil.get(CONSTANTS.LOGIN_REG_USER) != undefined)){
            return true;
        }
        commonUtil.jump("wap/regist/login.html");
        return false;
    };
    /**
     * 未实名用户去实名页面
     */
	commonUtil.needRealName = function(){
        var regUser = containerUtil.get(CONSTANTS.LOGIN_REG_USER);
        if(regUser.identify === 0){
     ...
     * wap端退出操作
     */
    commonUtil.wapLogout = function(){
        wapAjaxUtil.post("indexController/logout.do", {}, function(data){
            _cleanLoginCache();
            commonUtil.jump("wap/index/index.html");
        },null,false);
        return false;
    };
     ...
     * wap端session超时
     */
	commonUtil.wapSessionTimeOut = function(){
        _cleanLoginCache();
        commonUtil.jump("wap/regist/login.html");
    };
	/**
	...
	function _cleanLoginCache(){
        ...
        //清除wap端登录信息
        containerUtil.remove(CONSTANTS.LOGIN_REG_USER);
        //同步登录状态
    ...
    /**
	 * @function 提交按钮是否有效
     * @param _currentFlag 当前校验字段标识 true/或false
	 * @param _vilidSum 需要校验字段总数
	 * @param _submitClass_t 可以提交样式
	 * @param __submitClass_f 不可提交样式
     */
    commonUtil.changeSubmitState = function (_currentFlag,_vilidSum,_submitId,_submitClass_t,__submitClass_f) {
    	//校验标识总数
		var _sum = _vilidSum;
		var _tempSum = 0;
		var _currentValue =  _currentFlag == true ? 1 : 0;
        _tempSum = _tempSum + _currentValue;
        console.log("_currentFlag"+_currentFlag+" _tempSum:"+_tempSum);
        if(_tempSum == _sum){
            $("#"+_submitId).removeClass(__submitClass_f).addClass(_submitClass_t);
		}else {
            $("#"+_submitId).removeClass(_submitClass_t).addClass(__submitClass_f);
		}
    }
})();
