...
(function () {
    ...
    dictionaryUtil.initContract = function () {
        ...
        if (!containerUtil.get("contractTypeAndName") || !containerUtil.get("contractTypeAndName").resStatus == 999){
	...
	//省市转
    dictionaryUtil.getAreaProvinceAndCity = function(){
        ...
        for(var i in AREA_CONSTANT.data){
             var data = AREA_CONSTANT.data[i];
             if(data.grade == 1){
                 var city = dictionaryUtil.getAreaData(getCityCode(data.areaCode));
                 var AreaProvinceAndCity = {};
                 AreaProvinceAndCity.areaCode  = data.areaCode;
                 AreaProvinceAndCity.areaName  = data.areaName;
                 AreaProvinceAndCity.level = city;
                 dataCity.push(AreaProvinceAndCity);
			 }
        }
        return dataCity;
	};
	...
	dictionaryUtil.initAreaSelect = function(provinceName, cityName, countryName, currProvince, currCity, areaName){
		...
		if(currProvince != '' && currProvince != null && currProvince != undefined && currProvince.length > 0){
			_birthCity.append(dictionaryUtil.initAreaOption(currProvince, areaName));
		}
		_birthCountry.empty();
		_birthCountry.append('<option value="-999">--县--</option>');
		if(currCity != '' && currCity != null && currCity != undefined && currCity.length > 0){
